...
void Hydro::RiemannSolver(const int kl, const int ku, const int jl, const int ju,
  ...
  Real igm1 = 1.0/gm1;
	...
	//std::cout << "[RiemannSolver]: gamma: " << pmy_block->peos->GetGamma() << std::endl; 
...
//--- Step 6.  Compute L/R fluxes along the line bm, bp
    ...
    fr[IEN] = er*vxr + wri[IPR]*wri[IVX];
		...
		fl[IIE] = fl[IDN]*wli[IGE];
		fr[IIE] = fr[IDN]*wri[IGE]; 
...
// of the flux along the contact
    ...
    flxi[IEN] = sl*fl[IEN] + sr*fr[IEN] + sm*cp*am;
		// Fluxes of internal energy, computed from density flux
		if (flxi[IDN] >= 0) {
			flxi[IIE] = flxi[IDN]*wli[IGE];
		}
		else {
			flxi[IIE] = flxi[IDN]*wri[IGE];
		}
