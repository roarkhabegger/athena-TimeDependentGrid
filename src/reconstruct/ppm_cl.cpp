...
//         this file is for cless variables 
//
// REFERENCES:
...
//----------------------------------------------------------------------------------------
//! \fn Reconstruction::PiecewiseParabolicCLX1()
...
void Reconstruction::PiecewiseParabolicCLX1(MeshBlock *pmb,
  ...
  q_im2.InitWithShallowCopy(pmb->precon->scrcl2_ni_);
  q_im1.InitWithShallowCopy(pmb->precon->scrcl3_ni_);
  q.InitWithShallowCopy(pmb->precon->scrcl4_ni_);
  q_ip1.InitWithShallowCopy(pmb->precon->scrcl5_ni_);
  q_ip2.InitWithShallowCopy(pmb->precon->scrcl6_ni_);
  qr_imh.InitWithShallowCopy(pmb->precon->scrcl7_ni_);
  ql_iph.InitWithShallowCopy(pmb->precon->scrcl8_ni_);

  // set work PPM work arrays to shallow copies of scratch arrays:
  AthenaArray<Real> dd,dd_im1,dd_ip1,dph,dph_ip1;
  dd.InitWithShallowCopy(pmb->precon->scrcl02_i_);
  dd_im1.InitWithShallowCopy(pmb->precon->scrcl03_i_);
  dd_ip1.InitWithShallowCopy(pmb->precon->scrcl04_i_);
  dph.InitWithShallowCopy(pmb->precon->scrcl05_i_);
  dph_ip1.InitWithShallowCopy(pmb->precon->scrcl06_i_);

  AthenaArray<Real> d2qc_im1,d2qc,d2qc_ip1,d2qf;
  d2qc_im1.InitWithShallowCopy(pmb->precon->scrcl07_i_);
  d2qc.InitWithShallowCopy(pmb->precon->scrcl08_i_);
  d2qc_ip1.InitWithShallowCopy(pmb->precon->scrcl09_i_);
  d2qf.InitWithShallowCopy(pmb->precon->scrcl10_i_);

  AthenaArray<Real> qplus,qminus,dqf_plus,dqf_minus;
  qplus.InitWithShallowCopy(pmb->precon->scrcl11_i_);
  qminus.InitWithShallowCopy(pmb->precon->scrcl12_i_);
  dqf_plus.InitWithShallowCopy(pmb->precon->scrcl13_i_);
  dqf_minus.InitWithShallowCopy(pmb->precon->scrcl14_i_);
...
//----------------------------------------------------------------------------------------
//! \fn Reconstruction::PiecewiseParabolicCLX2()
...
void Reconstruction::PiecewiseParabolicCLX2(MeshBlock *pmb,
  ...
  bx.InitWithShallowCopy(pmb->precon->scrcl01_i_);
  wc.InitWithShallowCopy(pmb->precon->scrcl1_ni_);
  q_jm2.InitWithShallowCopy(pmb->precon->scrcl2_ni_);
  q_jm1.InitWithShallowCopy(pmb->precon->scrcl3_ni_);
  q.InitWithShallowCopy(pmb->precon->scrcl4_ni_);
  q_jp1.InitWithShallowCopy(pmb->precon->scrcl5_ni_);
  q_jp2.InitWithShallowCopy(pmb->precon->scrcl6_ni_);
  qr_jmh.InitWithShallowCopy(pmb->precon->scrcl7_ni_);
  ql_jph.InitWithShallowCopy(pmb->precon->scrcl8_ni_);

  // set work PPM work arrays to shallow copies of scratch arrays:
  AthenaArray<Real> dd,dd_jm1,dd_jp1,dph,dph_jp1;
  dd.InitWithShallowCopy(pmb->precon->scrcl02_i_);
  dd_jm1.InitWithShallowCopy(pmb->precon->scrcl03_i_);
  dd_jp1.InitWithShallowCopy(pmb->precon->scrcl04_i_);
  dph.InitWithShallowCopy(pmb->precon->scrcl05_i_);
  dph_jp1.InitWithShallowCopy(pmb->precon->scrcl06_i_);

  AthenaArray<Real> d2qc_jm1,d2qc,d2qc_jp1,d2qf;
  d2qc_jm1.InitWithShallowCopy(pmb->precon->scrcl07_i_);
  d2qc.InitWithShallowCopy(pmb->precon->scrcl08_i_);
  d2qc_jp1.InitWithShallowCopy(pmb->precon->scrcl09_i_);
  d2qf.InitWithShallowCopy(pmb->precon->scrcl10_i_);
...
//----------------------------------------------------------------------------------------
//! \fn Reconstruction::PiecewiseParabolicCLX3()
...
void Reconstruction::PiecewiseParabolicCLX3(MeshBlock *pmb,
  ...
  bx.InitWithShallowCopy(pmb->precon->scrcl01_i_);
  wc.InitWithShallowCopy(pmb->precon->scrcl1_ni_);
  q_km2.InitWithShallowCopy(pmb->precon->scrcl2_ni_);
  q_km1.InitWithShallowCopy(pmb->precon->scrcl3_ni_);
  q.InitWithShallowCopy(pmb->precon->scrcl4_ni_);
  q_kp1.InitWithShallowCopy(pmb->precon->scrcl5_ni_);
  q_kp2.InitWithShallowCopy(pmb->precon->scrcl6_ni_);
  qr_kmh.InitWithShallowCopy(pmb->precon->scrcl7_ni_);
  ql_kph.InitWithShallowCopy(pmb->precon->scrcl8_ni_);

  // set work PPM work arrays to shallow copies of scratch arrays:
  AthenaArray<Real> dd,dd_km1,dd_kp1,dph,dph_kp1;
  dd.InitWithShallowCopy(pmb->precon->scrcl02_i_);
  dd_km1.InitWithShallowCopy(pmb->precon->scrcl03_i_);
  dd_kp1.InitWithShallowCopy(pmb->precon->scrcl04_i_);
  dph.InitWithShallowCopy(pmb->precon->scrcl05_i_);
  dph_kp1.InitWithShallowCopy(pmb->precon->scrcl06_i_);

  AthenaArray<Real> d2qc_km1,d2qc,d2qc_kp1,d2qf;
  d2qc_km1.InitWithShallowCopy(pmb->precon->scrcl07_i_);
  d2qc.InitWithShallowCopy(pmb->precon->scrcl08_i_);
  d2qc_kp1.InitWithShallowCopy(pmb->precon->scrcl09_i_);
  d2qf.InitWithShallowCopy(pmb->precon->scrcl10_i_);
